...
def github_graphql(token: str, username: str, now: datetime | None = None) -> dict[str, Any]:
    ...
    if not isinstance(calendar.get("weeks"), list):
        raise RuntimeError("GitHub returned an invalid contribution calendar")
    return calendar
...
def render_svg(calendar: dict[str, Any], username: str, verdict: str) -> str:
    ...
    for weekday, label in {1: "Mon", 3: "Wed", 5: "Fri"}.items():
        ...
        parts.append(
            f'<text x="12" y="{y}" fill="{MUTED}" font-family="ui-monospace, SFMono-Regular, Menlo, Consolas, monospace" font-size="10">{label}</text>'
        )
    ...
    for week_index, week in enumerate(weeks):
        ...
        for day in week["contributionDays"]:
            ...
            fill = LEVELS.get(str(day.get("contributionLevel", "NONE")), EMPTY)
            date = escape(str(day["date"]))
            count = int(day["contributionCount"])
            parts.append(
                f'<rect x="{x}" y="{y}" width="{cell}" height="{cell}" rx="2" fill="{fill}"><title>{date}: {count} contributions</title></rect>'
            )

    legend_y = 174
    legend_x = width - 205
    parts.append(
        f'<text x="{legend_x - 38}" y="{legend_y + 9}" fill="{MUTED}" font-family="ui-monospace, SFMono-Regular, Menlo, Consolas, monospace" font-size="10">less</text>'
    )
    legend_colors = [
        EMPTY,
        LEVELS["FIRST_QUARTILE"],
        LEVELS["SECOND_QUARTILE"],
        LEVELS["THIRD_QUARTILE"],
        LEVELS["FOURTH_QUARTILE"],
    ]
    ...
    parts.append(
        f'<text x="{legend_x + 5 * (cell + gap) + 2}" y="{legend_y + 9}" fill="{MUTED}" font-family="ui-monospace, SFMono-Regular, Menlo, Consolas, monospace" font-size="10">more</text>'
    )
...
def main() -> int:
    ...
    print(f"Updated {OUTPUT_PATH.relative_to(ROOT)} with Mori verdict: {verdict}")
    return 0
